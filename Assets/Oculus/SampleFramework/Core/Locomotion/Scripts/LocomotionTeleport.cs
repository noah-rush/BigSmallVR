...
public class LocomotionTeleport : MonoBehaviour
{
		...
		switch (AimCollisionType)
		{
			case AimCollisionTypes.Capsule:
			{
				...
				bool hitOtherLayer = Physics.CapsuleCast(start + new Vector3(0, 0, 0),
					start + new Vector3(0, h, 0), r, direction,
					out otherLayersHit, distance, otherLayers, QueryTriggerInteraction.Ignore);
	...
	protected IEnumerator CancelAimStateCoroutine()
	{
		...
		// GameObject QuillonUI = GameObject.Find("QuillonUI");
		// if(leftHandObj != null){
		// 	leftHandObj.grabbedRigidbody.gameObject.layer = 8;
		// 	SetLayerRecursively(leftHandObj.grabbedRigidbody.gameObject, 8);
			
		// }
		// if(rightHandObj != null){
		// 	QuillonUI.transform.GetChild(0).GetComponent<TMP_Text>().text = rightHandObj.grabbedRigidbody.gameObject.name;
		// 	// QuillonUI.transform.GetChild(0).GetComponent<TextMeshPro>().text = rightHandObj.name;
		// 	SetLayerRecursively(rightHandObj.grabbedRigidbody.gameObject, 8);
		// 	// rightHandObj.grabbedRigidbody.gameObject.layer = 9;
		// 	// QuillonUI.transform.GetChild(0).GetComponent<TextMeshPro>().text = rightHandObj.gameObject.layer;
